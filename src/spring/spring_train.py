...
def train(args):
  ...
  model = Feedforward(args.input_dim,  args.hidden_dim, output_dim, baseline=args.baseline)
  ...
  for step in range(args.total_steps+1):
    ...
    # train step
    dxdt_hat = model.time_derivative(x.unsqueeze(-1)).squeeze(-1)
    loss = L2_loss(dxdt, dxdt_hat)
    loss.backward() ; optim.step() ; optim.zero_grad()
    
    # run test data
    test_dxdt_hat = model.time_derivative(test_x.unsqueeze(-1)).squeeze(-1)
  ...
  train_dxdt_hat = model.time_derivative(x.unsqueeze(-1)).squeeze(-1)
